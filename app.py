...
def formatted_date(original_date):
    date_object = datetime.strptime(original_date, "%a, %d %b %Y %H:%M:%S %Z")
    return date_object.strftime("%Y-%m-%d")

@app.route("/data_peserta")
def data_peserta():
    if "username" in session and session["role"] == "admin":
        # Mendapatkan tanggal
        current_date = formatted_date(session.get("current_date"))
        print(current_date)
        ...
        return render_template("data_peserta.html", login_data=login_data)
    else:
        return redirect("/")
...
def download():
    if "username" in session and session["role"] == "admin":
        ...
        response.headers["Content-Disposition"] = "attachment; filename=attendance_{}.xlsx".format(current_date)
